        ...
        function BuilderMonthBody(htb, showday, startday, events, bodyHeight) {
            ...
            for (var j = 0; j < rc; j++) {
              ...
              //  var titletemp = "<td class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\"><a href='javascript:void(0);' class='monthdayshow'>${dayshow}</a></td>";
                var titletemp = "<td class=\"st-dtitle${titleClass}\" ch='qkadd' abbr='${abbr}' axis='00:00' title=\"${title}\">"
                	+ "<span style='float:right;color:#1155CC'>${festivalShow}</span>"
                	+ "<a href='javascript:void(0);' class='monthdayshow'>${dayshow}</a></td>";
                	
                for (var i = 0; i < 7; i++) {
                    var o = { titleClass: "", dayshow: "" ,festivalShow :""};
                    var day = C[j * 7 + i];
                    
                    // - start - 扩展节日  @author wwl
                	if (true) {
                		//月视图天数数字显示
            			var cnMonth = CnMonthofDate(day);	//农历月
            			var cnDay = CnDayofDate(day);		//农历日
            			var solar = SolarTerm(day);		//农历节气
            			if(solar!='') cnDay=solar;
            			var cnMonDay = cnMonth+cnDay;	// 农历日期
            			...
            			switch(cnMonDay){
            				...
            				case '正月十五': holiday = '元宵节';break;
            				case '五月初五': holiday = '端午节';break;
            				case '七月初七': holiday = '七夕节';break;
            				case '八月十五': holiday = '中秋节';break;
            				case '九月初九': holiday = '重阳节';break;
            				case '腊月三十': holiday = '除夕';break; 
            				
            			}
            			
            			// 公历节日，按照指定格式在后面继续添加
            			switch(day.getMonth()+1+'/'+day.getDate()){
            			
	            		    case '1/1':holiday='元旦';break;
	            		    case '3/8':holiday='妇女节';break;
	            		    case '4/1':holiday='愚人节';break;
	            		    case '4/5':holiday='清明节';break;
	            		    case '5/1':holiday='劳动节';break;
	            		    case '5/4':holiday='青年节';break;
	            		    case '6/1':holiday='儿童节';break;
	            		    case '7/1':holiday='建党日';break;
	            		    case '8/1':holiday='建军节';break;
	            		    case '9/10':holiday='教师节';break;
	            		    case '10/1':holiday='国庆节';break;
	            		    case '12/24':holiday='平安夜';break;
	            		    case '12/25':holiday='圣诞节';break;
            		    
            			}
            			o.festivalShow = holiday;
                	}
                	// - end - 扩展节日  @author wwl
